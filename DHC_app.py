...
if "workbook_buffer" in st.session_state:
    ...
    st.caption("Optional — upload today's BRS MIS files to fill in the Delay in Receipting / Deposition section with real counts instead of [Fill manually].")
    ...
    if chq_brs_file is not None:
        try:
            chq_brs = etl.process_brs_for_mail(etl.load_brs_raw(chq_brs_file), "Cheque")
            st.caption(f"✓ Cheque/DD BRS loaded — Delay in Deposition: {chq_brs['delay']['delay_deposition']:,}, Delay in Receipting: {chq_brs['delay']['delay_receipting']:,}")
        except Exception as exc:
            st.error(f"Couldn't process the Cheque/DD BRS file: {exc}")

    air_brs = None
    if air_brs_file is not None:
        try:
            air_brs = etl.process_brs_for_mail(etl.load_brs_raw(air_brs_file), "Airtel")
            st.caption(f"✓ Airtel Cash BRS loaded — Delay in Deposition: {air_brs['delay']['delay_deposition']:,}, Delay in Receipting: {air_brs['delay']['delay_receipting']:,}")
        except Exception as exc:
            st.error(f"Couldn't process the Airtel Cash BRS file: {exc}")
