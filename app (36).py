...
with st.expander("ℹ️ About this platform", expanded=False):
    st.markdown("""
    Consolidates 4 monthly source files into 5 analytics summaries — RTGS turn-around time,
...
with st.container(border=True):
    step_header(1, "Upload Source Files", "All four files are required to run the pipeline.")
...
with st.container(border=True):
    ...
    all_files_ready = all([dcr_file, disable_file, employee_file, collection_mis_file])
    ...
    if run:
        try:
            with st.status("📊 Running automated pipeline...", expanded=True) as status:
                ...
                st.write("📥 **Stage 4/6**: Loading Collection MIS Base (CIF & DPD Slab)...")
...
if "workbook_buffer" in st.session_state:
    ...
    if export_choice == "Receipt Made Summary":
        rms_table = st.session_state["receipt_made_summary"]["right"]
