...
if "workbook_buffer" in st.session_state:
    ...
    col_mail, col_mail_info = st.columns([2, 1])
    ...
    with col_mail:
        ...
        st.link_button("📧 Compose Summary Mail", mailto_url, use_container_width=True, type="primary")
        with st.expander("Preview mail content"):
            st.text(f"Subject: {subject}\n\n{body}")

    with col_mail_info:
        st.info("""
        **Opens your default mail app**
        (Outlook, if it's set as default)
        with a ready-made draft — one
        short, spaced-out summary per
        report. Attach the workbook
