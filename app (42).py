...
if "workbook_buffer" in st.session_state:
    ...
    subject, body = etl.build_mail_summary(
        st.session_state["receipt_made_summary"],
        st.session_state["rtgs_summary"],
        st.session_state["cash_mode_validation"],
        st.session_state["delay_summary"],
    ...
    with col_mail1:
        if st.button("🖼️ Compose in Outlook (with charts)", use_container_width=True, type="primary"):
            try:
                ...
                rms_path = os.path.join(tmp_dir, "receipt_made_report.png")
                rtgs_path = os.path.join(tmp_dir, "rtgs_summary.png")

                with open(rms_path, "wb") as f:
                    f.write(etl.receipt_made_status_summary_to_image_bytes(
                        st.session_state["receipt_made_summary"]["full"]
                    ).getvalue())
                with open(rtgs_path, "wb") as f:
                    f.write(etl.rtgs_summary_to_image_bytes(st.session_state["rtgs_summary"]).getvalue())
                ...
                etl.compose_outlook_mail_with_images(
                    html_subject, html_body,
                    [("receipt_made_report", rms_path), ("rtgs_summary", rtgs_path)],
                )
    ...
    with col_mail_info:
        ...
        **With charts** opens classic
        ...
        the Receipt Made Report and RTGS
        Summary images embedded inline.
