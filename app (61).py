...
if st.session_state["active_user"] in ("shalini", "bhoobalan"):
    ...
    st.markdown(f"""
    <div class="header-container" style="text-align:center;">
        <h1 class="header-title">💰 {active['icon']} {active['label']} — Collection Operations</h1>
        <p class="header-subtitle">Pay Mode TAT Validation &amp; Airtel Gateway Collection</p>
    </div>
    """, unsafe_allow_html=True)

    shalini_feature = st.selectbox(
        "Select Feature",
        ["Pay Mode Exceeding", "Airtel Gateway Collection"],
        key="shalini_feature",
    )

    if shalini_feature == "Pay Mode Exceeding":
        import urllib.parse as _urlparse
        st.subheader("TAT Validation — Pay Mode Exceeding")
        mode = st.radio("Validation filter", ["Cash", "Cheque", "DD"], horizontal=True)
        uploaded_dcr = st.file_uploader("Upload DCR (.xlsb)", type=["xlsb"], key="shalini_dcr")
        if not uploaded_dcr:
            st.info("Upload a DCR file to begin.")
            st.stop()
        current_sig = (uploaded_dcr.name, uploaded_dcr.size, mode)
        if st.button("▶️ Click to Process", type="primary", use_container_width=True, key="shalini_process"):
            try:
                df = etl.load_dcr_raw(uploaded_dcr)
                result_df, meta = etl.filter_mode_exceeding(df, mode)
                st.session_state["shalini_tat"] = {"sig": current_sig, "result_df": result_df, "meta": meta}
            except Exception as exc:
                st.error(str(exc)); st.stop()
        stored = st.session_state.get("shalini_tat")
        if stored is None:
            st.info("Click **Click to Process** to run the TAT check."); st.stop()
        if stored["sig"] != current_sig:
            st.warning("File or mode changed — click Process again."); st.stop()
        result_df, meta = stored["result_df"], stored["meta"]
        st.caption(f"TAT: **{meta['threshold']} day(s)** · {meta['total_matched']:,} {mode} receipts · {meta['total_exceeding']:,} exceeding TAT")
        if meta["missing_columns"]:
            st.warning("Missing columns: " + ", ".join(meta["missing_columns"]))
        st.metric(f"{mode} receipts exceeding TAT", meta["total_exceeding"])
        if result_df.empty:
            st.success("No receipts exceeding TAT."); st.stop()
        st.dataframe(result_df, use_container_width=True, hide_index=True)
        st.markdown("---")
        st.markdown("### 📧 Email This List")
        subj_html, html_body = etl.build_tat_mail_html(mode, result_df)
        c1, c2 = st.columns(2)
        with c1:
            if st.button("🖼️ Compose in Outlook (full list)", use_container_width=True, key="shalini_outlook"):
                try:
                    etl.compose_outlook_mail_html(subj_html, html_body)
                    st.success("Draft opened in Outlook.")
                except ImportError:
                    st.error("Needs pywin32: run `pip install pywin32`.")
                except Exception as exc:
                    st.error(f"Couldn't open Outlook: {exc}")
        with c2:
            subj_txt, body_txt = etl.build_tat_mail_text(mode, result_df)
            mailto_url = "mailto:?subject=" + _urlparse.quote(subj_txt) + "&body=" + _urlparse.quote(body_txt)
            st.link_button("📧 Compose (text only)", mailto_url, use_container_width=True)

    else:  # Airtel Gateway Collection
        st.subheader("Airtel Money Gateway Collection Dashboard")
        uploaded_dcr2 = st.file_uploader("Upload DCR (.xlsb)", type=["xlsb"], key="shalini_airtel_dcr")
        if not uploaded_dcr2:
            st.info("Upload a DCR file to begin."); st.stop()
        if st.button("▶️ Generate Dashboard", type="primary", use_container_width=True, key="shalini_airtel_process"):
            try:
                df = etl.load_dcr_raw(uploaded_dcr2)
                zone_df, region_df = etl.generate_airtel_dashboard(df)
                st.session_state["shalini_airtel_zone"] = zone_df
                st.session_state["shalini_airtel_region"] = region_df
            except Exception as exc:
                st.error(f"Error: {exc}"); st.stop()
        if "shalini_airtel_zone" not in st.session_state:
            st.info("Click **Generate Dashboard** to process."); st.stop()
        zone_df = st.session_state["shalini_airtel_zone"]
        region_df = st.session_state["shalini_airtel_region"]
        grand_bank = zone_df.loc[zone_df[("Zone", "")] == "Grand Total", ("Deposited - Bank", "Value")].values[0]
        grand_all  = zone_df.loc[zone_df[("Zone", "")] == "Grand Total", ("Overall", "Value")].values[0]
        bank_pct = (grand_bank / grand_all * 100) if grand_all else 0
        st.warning(f"⚠️ **{bank_pct:.2f}%** of Cash deposited in Bank.")
        def _style(df):
            fmt = {}
            for col in df.columns:
                if isinstance(col, tuple):
                    if col[1] in ["Count %", "Value %"]: fmt[col] = "{:,.2f}%"
                    elif col[1] == "Count": fmt[col] = "{:,.0f}"
                    elif col[1] == "Value": fmt[col] = "{:,.2f}"
            return df.style.format(fmt)
        st.write("### Zone-wise Dashboard")
        st.dataframe(_style(zone_df), use_container_width=True, hide_index=True)
        st.divider()
        st.write("### Region-wise Dashboard")
        st.dataframe(_style(region_df), use_container_width=True, hide_index=True)
        st.markdown("---")
        st.markdown("### 📧 Email This Dashboard")
        if st.button("🖼️ Draft Airtel Dashboard in Outlook", use_container_width=True, key="shalini_airtel_mail"):
            try:
                subj, html_body = etl.build_airtel_mail_html(zone_df, region_df, bank_pct)
                etl.compose_outlook_mail_html(subj, html_body)
                st.success("Draft opened in Outlook.")
            except ImportError:
                st.error("Needs pywin32: run `pip install pywin32`.")
            except Exception as exc:
...
with st.container(border=True):
    ...
    with brs_col1:
        ...
        if chq_brs_file is not None:
            try:
                chq_brs_raw_df = etl.load_brs_raw(chq_brs_file)
                st.session_state["chq_brs_raw"] = chq_brs_raw_df
                st.session_state["chq_brs"] = etl.process_brs_for_mail(chq_brs_raw_df, "Cheque")
                ...
                st.download_button(
                    "⬇️ Download Cheque/DD BRS Status (.xlsx)",
                    data=etl.brs_status_to_excel_bytes(cb).getvalue(),
                    file_name=f"Cheque_DD_BRS_Status_{datetime.now().strftime('%d%b%Y')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="chq_brs_download",
                )
            except Exception as exc:
                st.session_state.pop("chq_brs", None)
                st.session_state.pop("chq_brs_raw", None)
    ...
    with brs_col2:
        ...
        if air_brs_file is not None:
            try:
                air_brs_raw_df = etl.load_brs_raw(air_brs_file)
                st.session_state["air_brs_raw"] = air_brs_raw_df
                st.session_state["air_brs"] = etl.process_brs_for_mail(air_brs_raw_df, "Airtel")
                ...
                st.download_button(
                    "⬇️ Download Airtel Cash BRS Status (.xlsx)",
                    data=etl.brs_status_to_excel_bytes(ab).getvalue(),
                    file_name=f"Airtel_Cash_BRS_Status_{datetime.now().strftime('%d%b%Y')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="air_brs_download",
                )
            except Exception as exc:
                st.session_state.pop("air_brs", None)
                st.session_state.pop("air_brs_raw", None)
                st.error(f"Couldn't process this file: {exc}")
        else:
            st.session_state.pop("air_brs", None)
            st.session_state.pop("air_brs_raw", None)

        cash_brs_file = st.file_uploader("📑 Cash BRS MIS (.xlsx)", type=["xlsx"], key="cash_brs_upload")
        if cash_brs_file is not None:
            try:
                st.session_state["cash_brs"] = etl.load_cash_mis(cash_brs_file)
                try:
                    cash_brs_file.seek(0)
                    st.session_state["cash_brs_raw"] = etl.load_cash_brs_raw(cash_brs_file)
                except Exception:
                    # Zone-filtered Cash BRS just won't be available for this
                    # file (e.g. no "DATA" sheet) — the unfiltered "Overall"
                    # mail still works fine off the MIS sheet above.
                    st.session_state.pop("cash_brs_raw", None)
                cb2 = st.session_state["cash_brs"]
                st.caption(f"✓ Loaded — Delay in Deposition: {cb2['delay']['delay_deposition']:,}, Delay in Receipting: {cb2['delay']['delay_receipting']:,}")
                st.download_button(
                    "⬇️ Download Cash BRS Status (.xlsx)",
                    data=etl.brs_status_to_excel_bytes(cb2).getvalue(),
                    file_name=f"Cash_BRS_Status_{datetime.now().strftime('%d%b%Y')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="cash_brs_download",
                )
            except Exception as exc:
                st.session_state.pop("cash_brs", None)
                st.session_state.pop("cash_brs_raw", None)
                st.error(f"Couldn't process Cash BRS file: {exc}")
        else:
            st.session_state.pop("cash_brs", None)
            st.session_state.pop("cash_brs_raw", None)
...
if "workbook_buffer" in st.session_state:
    ...
    zone_choice = st.selectbox(
        "Draft for zone",
        ["Overall", "North", "South", "East", "West", "Head Office"],
        key="mail_zone_choice",
        help=(
            "Filters the whole mail — every count and every chart — down to one broad zone. "
            "'Overall' uses all zones. 'Head Office' is a separate, cross-cutting filter for "
            "rows tagged HE (e.g. 'SOUTH_1 HE') regardless of which geographic zone they're in."
        ),
    )

    chq_brs_raw = st.session_state.get("chq_brs_raw")
    air_brs_raw = st.session_state.get("air_brs_raw")
    cash_brs_raw = st.session_state.get("cash_brs_raw")
    loaded_brs = [n for n, v in (("Cheque/DD", chq_brs_raw), ("Airtel Cash", air_brs_raw), ("Cash", cash_brs_raw)) if v is not None]
    if loaded_brs:
        st.caption(f"✓ Using BRS MIS file(s) uploaded in Step 1: {', '.join(loaded_brs)}. Re-upload there to refresh.")
    else:
    ...
    if zone_choice == "Overall":
        # Unfiltered mail keeps using the pre-aggregated Step-1 results as-is
        # (Cash in particular is built from its pre-built "MIS" sheet, which
        # may carry manual adjustments beyond what's in the raw rows).
        chq_brs = st.session_state.get("chq_brs")
        air_brs = st.session_state.get("air_brs")
        cash_brs = st.session_state.get("cash_brs")
    else:
        # Zone-filtered mail: re-filter each BRS file's raw rows by the same
        # "ZONE" column (combining sub-zones like NORTH1/NORTH2 into NORTH,
        # same as the DCR) and recompute from that subset.
        chq_brs = etl.process_brs_for_mail(etl.filter_by_broad_zone(chq_brs_raw, "ZONE", zone_choice), "Cheque") if chq_brs_raw is not None else None
        air_brs = etl.process_brs_for_mail(etl.filter_by_broad_zone(air_brs_raw, "ZONE", zone_choice), "Airtel") if air_brs_raw is not None else None
        cash_brs = etl.process_cash_brs_for_mail(etl.filter_by_broad_zone(cash_brs_raw, "ZONE", zone_choice)) if cash_brs_raw is not None else None
        ...
        if st.button("🖼️ Compose in Outlook (with charts)", use_container_width=True, type="primary"):
            try:
                ...
                if chq_brs is not None:
                    images["chq_brs_status"] = etl.brs_status_image_bytes_v2(chq_brs).getvalue()
                if air_brs is not None:
                    images["air_brs_status"] = etl.brs_status_image_bytes_v2(air_brs).getvalue()
                if cash_brs is not None:
                    images["cash_brs_status"] = etl.brs_status_image_bytes_v2(cash_brs).getvalue()
