...
def build_executive_summary_facts(
    dcr_tab: pd.DataFrame,
    rtgs_tab: pd.DataFrame,
    rtgs_summary: dict,
    receipt_made_summary_by_status: pd.DataFrame,
    cash_mode_validation_summary: pd.DataFrame,
    rcpt_cxn: pd.DataFrame,
) -> dict:
    """
    Pulls a compact set of already-computed figures together for the AI
    executive summary. This function does NOT calculate anything new — it
    only reads totals that other functions in this file already produced
    and verified. This dict is the single source of truth an AI summary
    is allowed to reference; nothing outside it should ever appear in the
    generated text (see generate_executive_summary_text in app.py, which
    enforces this with a post-generation check).
    """
    grand = rtgs_summary["matrix"]["grand_total"]
    rms = receipt_made_summary_by_status
    grand_row = rms[rms["Mode"] == "Grand Total"].iloc[0] if not rms.empty else None

    high_value_count = int((cash_mode_validation_summary["Grand Total"] > HIGH_VALUE_THRESHOLD).sum()) if not cash_mode_validation_summary.empty else 0
    cash_violation_value = float(cash_mode_validation_summary["Grand Total"].sum()) if not cash_mode_validation_summary.empty else 0.0

    cxn_by_mode = {}
    if not rcpt_cxn.empty:
        amt = pd.to_numeric(rcpt_cxn["Amount"], errors="coerce").fillna(0)
        cxn_by_mode = (
            rcpt_cxn.assign(_amt=amt).groupby("PaymentMode", dropna=False)["_amt"]
            .agg(["size", "sum"]).rename(columns={"size": "count", "sum": "amount"})
            .to_dict("index")
        )

    return {
        "total_receipts": int(len(dcr_tab)),
        "rtgs_receipt_count": int(len(rtgs_tab)),
        "rtgs_updated_count": int(grand["total_count"]),
        "rtgs_value_cr": round(float(grand["total_value"]), 2),
        "rtgs_tat_less_than_4": int(grand["by_bucket"]["Less then 4"]["count"]),
        "rtgs_tat_5_to_10": int(grand["by_bucket"]["5 TO 10"]["count"]),
        "rtgs_tat_greater_than_10": int(grand["by_bucket"]["Great then 10"]["count"]),
        "receipts_cleared": int(grand_row["Cleared"]) if grand_row is not None and "Cleared" in rms.columns else 0,
        "receipts_pending": int(grand_row["Pending"]) if grand_row is not None and "Pending" in rms.columns else 0,
        "receipts_cancelled": int(grand_row["Cancelled"]) if grand_row is not None and "Cancelled" in rms.columns else 0,
        "cash_violations_count": int(len(cash_mode_validation_summary)),
        "cash_violations_value": round(cash_violation_value, 2),
        "cash_violations_high_value_count": high_value_count,
        "high_value_threshold": HIGH_VALUE_THRESHOLD,
        "cancelled_receipts_total": int(len(rcpt_cxn)),
        "cancelled_by_mode": {
            str(mode): {"count": int(v["count"]), "amount": round(float(v["amount"]), 2)}
            for mode, v in cxn_by_mode.items()
        },
    }
...
def cash_mode_validation_summary_to_excel_bytes(df: pd.DataFrame) -> io.BytesIO:
    """
    Same violations data as before, with one addition: a 'High-Value
    Violations' detail block up top listing every customer whose Grand
    Total cash payment exceeds ₹1,95,000 — full detail (CIF, Zone, Sub
    Region, Slab, every date column, Grand Total), sorted highest first,
    so the priciest violations aren't buried in the full list. The
    complete violations table (everyone, unfiltered) still follows below
    it — nothing is removed, this just surfaces the high-value ones too.
    """
    wb = Workbook()
    wb.remove(wb.active)
    ...
    if high_value.empty:
        _set(ws, r, 1, f"No customer exceeds ₹{HIGH_VALUE_THRESHOLD:,} in this period.", font=BODY_FONT)
        r += 1
    else:
        r = _write_df(ws, high_value, start_row=r) + 2

    r += 1
    _set(
        ws, r, 1, f"ALL VIOLATIONS — every flagged customer ({len(df)})",
        font=HEADER_FONT, fill=HEADER_FILL,
    )
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=len(df.columns))
    r += 1
    _write_df(ws, df.sort_values("Grand Total", ascending=False), start_row=r)
