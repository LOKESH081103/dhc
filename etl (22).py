...
_RECEIPT_MADE_STATUS_COLS = ["Cleared", "Deposit", "Bounced", "Cxn"]
_RECEIPT_MADE_STATUS_LABELS = {"Cleared": "CLEARED", "Deposit": "DEPOSITED", "Bounced": "BOUNCED", "Cxn": "CXN"}
...
def receipt_made_status_table_to_dataframe(table: dict) -> pd.DataFrame:
    ...
    Payment Mode x Cleared/Deposited/Bounced/CXN) into a plain DataFrame
    for the Streamlit preview — Receipt Status repeats per mode row rather
    than being visually merged, since st.dataframe can't merge cells, but
    the numbers and row order match the reference report exactly.
    """
    rows = []
    for group in table["groups"]:
        for r in group["rows"]:
            ...
            for c in _RECEIPT_MADE_STATUS_COLS:
                row[_RECEIPT_MADE_STATUS_LABELS[c]] = r["counts"][c]
    ...
    for c in _RECEIPT_MADE_STATUS_COLS:
        grand[_RECEIPT_MADE_STATUS_LABELS[c]] = table["grand_totals"][c]
...
def receipt_made_status_summary_to_excel_bytes(table: dict) -> io.BytesIO:
    ...
    for col in range(1, 2 + len(status_cols) + 1):
    ...
    for group in table["groups"]:
        ...
        for r in group["rows"]:
            ...
            for k, c in enumerate(status_cols):
                val = r["counts"][c]
                cell = ws.cell(row, 3 + k, val if val else None)
                cell.alignment = center
...
def receipt_made_status_summary_to_image_bytes(table: dict) -> io.BytesIO:
    ...
    status_colors = {"Cleared": (39, 174, 96), "Deposit": (41, 128, 185), "Bounced": (192, 57, 43), "Cxn": (230, 126, 34)}
    mode_colors = {
