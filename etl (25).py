...
def build_cash_mode_validation_summary(dcr_tab: pd.DataFrame) -> pd.DataFrame:
    ...
    paid in cash — and Airtel counts as cash here too, since it's a
    cash-collection channel rather than a separate payment method for
    compliance purposes. Primarily uses the mapped Mode column (AIRTEL
    and CASH raw codes already fold into "AIRTEL / CASH" via MODE_MAP),
    plus a raw-text fallback that catches any Airtel/Cash-worded
    MODEOFPAYMENT variant MODE_MAP doesn't recognise yet, so a mapping
    gap can never silently exclude an Airtel receipt from this check.
    Everything else about the report (columns, pivot, grouping) is
    unchanged.
    """
