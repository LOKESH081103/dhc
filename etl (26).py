...
def build_receipt_made_summary(dcr_tab: pd.DataFrame) -> dict:
    """
    Three views, all grouped from the same RECEIPT STATUS field:
    left = Updated/Pending x (Cleared, Deposited, Pending);
    right = Updated/Bounced-or-Cancelled x (Cleared, Deposited, Bounced, Cxn);
    full = Updated/Bounced-or-Cancelled x (Cleared, Deposited, Pending,
    Bounced, Cxn) — same two RECEIPT STATUS groups as `right` (Pending is
    a Status IN TAB value — STATUS_MAP's "NA" code — not a RECEIPT STATUS
    of its own, so it isn't a third row group), just with a Pending
    column added alongside Cleared/Deposited/Bounced/Cxn so any receipt
    marked Pending within Updated or Bounced/Cancelled still shows up.
    ...
    full = _status_table(d, ["UPDATED", "BOUNCED/CANCELLED"], _RECEIPT_MADE_STATUS_COLS)
...
def receipt_made_status_summary_to_image_bytes(table: dict) -> io.BytesIO:
    ...
    for group in table["groups"]:
        height += section_header_h + len(group["rows"]) * (card_h + card_gap) + 14
    height += 50
    ...
    for group in table["groups"]:
        ...
        gt = sum(r["total"] for r in group["rows"])
...
def cash_mode_validation_summary_to_image_bytes(df: pd.DataFrame) -> io.BytesIO:
    ...
    hv_mask = df["Grand Total"] > HIGH_VALUE_THRESHOLD
    hv_count = int(hv_mask.sum())
    hv_amount = float(df.loc[hv_mask, "Grand Total"].sum())
    ...
    draw.text((pad, 60), "Customers on the disable list who paid via restricted mode", font=_img_font(12), fill=(190, 200, 215))
    draw.text((pad, 82), datetime.now().strftime("%d-%b-%Y %H:%M"), font=_img_font(10), fill=(150, 165, 185))
    pill_text = f"{hv_amount_pct:.0f}% OF VALUE IS HIGH-RISK"
    ...
    kpis = [
        ...
        ("TOTAL AMOUNT", f"₹{total_amount:,.0f}", teal),
        ("HIGH-VALUE (>₹1.95L)", f"{hv_count:,}", gold),
        ("HIGH-VALUE AMOUNT", f"₹{hv_amount:,.0f}", crimson),
    ]
    kpi_gap = 16
    kpi_w = (width - 2 * pad - kpi_gap * 3) // 4
    x = pad
    ...
    draw.text((lx + 22, ly - 1), "High-Value", font=_img_font(12), fill=ink)
    draw.text((lx + 22, ly + 15), f"₹{hv_amount:,.0f}", font=_img_font(11), fill=(110, 110, 110))
    ly += 46
    draw.rectangle([lx, ly, lx + 14, ly + 14], fill=teal)
    draw.text((lx + 22, ly - 1), "Standard", font=_img_font(12), fill=ink)
    ...
    for i, (_, r) in enumerate(top5.iterrows()):
        ...
        draw.text((pad + 70, ry + 28), f"{r.get('Zone2', '')} / {r.get('Sub Region2', '')}  ·  Slab {r.get('Slab', '')}", font=_img_font(10), fill=(130, 130, 130))
...
CASH_RECEIPT_LIMIT = 195000


def build_cash_mode_validation_summary(dcr_tab: pd.DataFrame) -> pd.DataFrame:
    """
    Compliance check: cash-mode (Airtel/Cash) receipts where the total
    cash collected from a single CIF on a single day crosses the
    Rs 1,95,000 threshold — a buffer just under the Income Tax Act
    Section 269ST limit (Rs 2,00,000 received in cash from one person in
    a day). Flags on the day's cumulative amount, not the period's Grand
    Total, so an ordinary repeat customer whose receipts add up over
    weeks/months doesn't get flagged for something that never happened on
    any single day.

    This replaces an earlier version that checked disable-list membership
    with no amount check at all — it would flag a single ₹1,94,500 cash
    receipt (under the limit) as a "violation" just because that CIF was
    on a disable list, while a genuine over-limit cash day for a CIF not
    on any disable list was never flagged at all. That was the wrong
    rule entirely: "Cash Mode Validation" here means the Rs 1,95,000
    cash-receipt-limit check, not a disable-list check.
    ...
    pivot = pd.pivot_table(
        cash, index=["CIF", "Zone", "Sub Region", "Slab"],
    ...
    # Violation = some single day's cumulative cash collection for that
    # CIF crosses Rs 1,95,000 — not the running period total.
    exceeds_on_some_day = (pivot[date_cols].fillna(0) > CASH_RECEIPT_LIMIT).any(axis=1)
    pivot = pivot[exceeds_on_some_day]
...
HIGH_VALUE_THRESHOLD = 195000  # ₹1.95 lakh


def _write_cash_mode_validation_sheet(ws, df: pd.DataFrame):
    """
    Cash Mode Validation Summary — shows ONLY the high-value violations
    (Grand Total > ₹1,95,000), sorted highest first. The full unfiltered
    violations list used to also be shown below this; per request, this
    sheet now surfaces just the high-value cut, nothing else.
    """
    ws.cell(row=1, column=1, value="Cash Mode Validation Summary — High-Value Violations").font = TITLE_FONT
    ws.cell(row=2, column=1, value=f"Grand Total > ₹{HIGH_VALUE_THRESHOLD:,}  ·  Generated {datetime.now().strftime('%d-%b-%Y %H:%M')}").font = Font(italic=True, size=9, color="666666")

    if df.empty:
        ws.cell(row=4, column=1, value="No violations detected — fully compliant").font = BODY_FONT
        return

    high_value = df[df["Grand Total"] > HIGH_VALUE_THRESHOLD].sort_values("Grand Total", ascending=False)

    if high_value.empty:
        ws.cell(row=4, column=1, value=f"No customer exceeds ₹{HIGH_VALUE_THRESHOLD:,} in this period.").font = BODY_FONT
        return

    _set(
        ws, 4, 1,
        f"HIGH-VALUE VIOLATIONS — Grand Total > ₹{HIGH_VALUE_THRESHOLD:,}  ({len(high_value)} customers)",
        font=Font(name="Calibri", bold=True, size=10, color="FFFFFF"),
        fill=PatternFill("solid", start_color="C0392B"),
    )
    ws.merge_cells(start_row=4, start_column=1, end_row=4, end_column=len(high_value.columns))
    _write_df(ws, high_value, start_row=5)
...
def rcpt_cxn_to_excel_bytes(df: pd.DataFrame) -> io.BytesIO:
    """
    Same cancelled-receipt data as before, restructured: a Mode-wise
    summary block up top, then the detail rows grouped under a header band
    per payment mode (with a subtotal), instead of one flat table.
    """
    wb = Workbook()
    wb.remove(wb.active)
    ws = wb.create_sheet("RCPT CXN")
