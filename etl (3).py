...
def load_dcr(file) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # IMPORTANT: keep_default_na=False is required here. STATUS IN TAB uses
    # the literal code "NA" to mean Pending. Pandas' default NA-value list
    # (used on every Excel/CSV read) includes the bare text "NA" and
    # silently converts it to a real missing value — so every Pending
    # receipt was vanishing before it ever reached STATUS_MAP, while
    # looking, on the surface, like only "Cleared" was being tracked.
    # keep_default_na=False stops that; genuinely empty cells still come
    # through as real blanks from the binary format itself (not as text),
    # so blank-detection elsewhere is unaffected.
    receipts = pd.read_excel(file, sheet_name="Sheet1", engine="pyxlsb", keep_default_na=False)
    # Restore normal blank semantics for every other column: an actual
    # empty string (not literal "NA") should still count as missing.
    receipts = receipts.replace("", np.nan)
...
def receipt_made_summary_to_image_bytes(df: pd.DataFrame) -> io.BytesIO:
    """
    Renders the same table as a clean report-card PNG — deliberately not
    styled like the Excel grid (dark banner, colour-coded status dots,
    highlighted total row) so it's immediately distinguishable and reads
    well dropped straight into a chat message or a slide.
    """
    ...
    mode_w, total_w = 230, 140
    stat_w = max(120, 620 // max(len(status_cols), 1))
    left_pad = 36
    header_h, col_header_h, row_h = 130, 46, 56
    width = left_pad * 2 + mode_w + stat_w * len(status_cols) + total_w
    height = header_h + col_header_h + row_h * len(df) + 30

    img = Image.new("RGB", (width, height), (247, 249, 251))
    draw = ImageDraw.Draw(img)

    draw.rectangle([0, 0, width, header_h], fill=(19, 44, 79))
    draw.text((left_pad, 28), "RECEIPT MADE SUMMARY", font=font(28), fill="white")
    draw.text((left_pad, 68), "By Payment Mode & Receipt Status", font=font(14), fill=(190, 205, 222))
    draw.text(
        (left_pad, 92),
        f"Source: STATUS IN TAB (C/NA/X)  ·  {datetime.now().strftime('%d-%b-%Y %H:%M')}",
        font=font(12), fill=(150, 168, 191),
    )

    y = header_h
    draw.rectangle([0, y, width, y + col_header_h], fill=(233, 236, 239))
    x = left_pad
    draw.text((x, y + 13), "MODE", font=font(15), fill=(40, 40, 40))
    x += mode_w
    for sc in status_cols:
        draw.text((x + 10, y + 13), str(sc).upper(), font=font(15), fill=(40, 40, 40))
        x += stat_w
    draw.text((x + 10, y + 13), "TOTAL", font=font(15), fill=(40, 40, 40))
    y += col_header_h

    for i, (_, r) in enumerate(df.iterrows()):
        is_total = str(r["Mode"]) == "Grand Total"
        row_bg = (222, 231, 242) if is_total else ((255, 255, 255) if i % 2 == 0 else (247, 249, 251))
        draw.rectangle([0, y, width, y + row_h], fill=row_bg)

        x = left_pad
        draw.text((x, y + 18), str(r["Mode"]), font=font(16), fill=(20, 20, 20))
        x += mode_w
        for sc in status_cols:
            val = int(r[sc])
            color = status_colors.get(sc, (100, 100, 100))
            draw.ellipse([x, y + 24, x + 12, y + 36], fill=color)
            draw.text((x + 20, y + 18), f"{val:,}", font=font(16), fill=(30, 30, 30))
            x += stat_w
        draw.text((x + 10, y + 18), f"{int(r['Total']):,}", font=font(17), fill=(19, 44, 79))

        y += row_h
        draw.line([0, y, width, y], fill=(225, 228, 231), width=1)
