...
def build_mail_summary(
    ...
    """
    Builds (subject, body) summarising all five reports — Receipt Made,
    RTGS, Cash Mode Validation, Delay in Recepting, and Receipt
    Cancellation — one after another, in a few words each, with generous
    spacing so it doesn't read cramped in a mail client. Pulls straight
    from the same summary dicts/DataFrames already in st.session_state,
    so the numbers always match the exported Excel files.
    """
    date_str = report_date or datetime.now().strftime("%d-%b-%Y")
    subject = f"DHC Working Automation — MIS Summary — {date_str}"

    # 1) Receipt Made Summary — the reconciled "full" table (Updated /
    # Pending / Bounced-or-Cancelled together), so nothing is left out.
    rms_full = receipt_made_summary["full"]
    rms_lines = [
        f"   - {group['status'].title()}: {sum(group['totals'].values()):,} receipt(s)"
        for group in rms_full["groups"]
    ]

    # 2) RTGS Summary — Updated-only TAT performance (matches the RTGS
    # Summary sheet's own filter).
    rtgs_grand = rtgs_summary["matrix"]["grand_total"]
    rtgs_lines = [
        f"   - {TAT_DISPLAY[b]}: {rtgs_grand['by_bucket'][b]['count']:,}"
        for b in TAT_ORDER
    ]

    # 3) Cash Mode Validation — just the headline count; details are in
    # the attached workbook.
    cmv_count = len(cash_mode_validation_df)

    # 4) Delay in Recepting — full-month aging, same TAT buckets as RTGS.
    delay_grand = delay_summary["matrix"]["grand_total"]
    delay_lines = [
        f"   - {TAT_DISPLAY[b]}: {delay_grand['by_bucket'][b]['count']:,}"
        for b in TAT_ORDER
    ]

    # 5) Receipt Cancellation register — headline count only.
    cxn_count = len(rcpt_cxn_df)

    body = "\n".join([
        "Hi,",
        "",
        f"Quick summary of today's DCR processing ({date_str}). Full workbook attached separately.",
        "",
        "1. RECEIPT MADE SUMMARY",
        *rms_lines,
        f"   - Grand Total: {rms_full['grand_total']:,} receipt(s)",
        "",
        "2. RTGS SUMMARY",
        f"   - Total Updated RTGS Receipts: {rtgs_grand['total_count']:,}",
        *rtgs_lines,
        f"   - Total Value: Rs {rtgs_grand['total_value']:.2f} Cr",
        "",
        "3. CASH MODE VALIDATION SUMMARY",
        f"   - {cmv_count:,} violation(s) found" if cmv_count else "   - No violations found. Fully compliant.",
        "",
        "4. DELAY IN RECEPTING SUMMARY",
        f"   - Total Receipts: {delay_grand['total_count']:,}",
        *delay_lines,
        "",
        "5. RECEIPT CANCELLATION SUMMARY",
        f"   - {cxn_count:,} receipt(s) cancelled this period" if cxn_count else "   - No cancelled receipts this period.",
        "",
        "Regards,",
        "DHC Working Automation",
    ])
    return subject, body
...
def build_mail_html_summary(
    ...
    """
    HTML counterpart of build_mail_summary, for the "Compose in Outlook
    (with charts)" button. Same five sections in the same order, but as
    HTML — with the Receipt Made Report dashboard image sitting right
    under section 1, and the RTGS Summary dashboard image (which already
    includes its own Online Payment Channels panel — there's no separate
    online-payment export) right under section 2, so each picture sits
    next to the text it illustrates instead of piled at the end.

    The <img> tags reference cid: URIs — the caller (see
    compose_outlook_mail_with_images) must attach the two PNGs with
    matching Content-IDs (receipt_made_cid / rtgs_cid) for the images to
    actually show up once this is set as an Outlook MailItem's HTMLBody.
    """
    subject, _ = build_mail_summary(
        receipt_made_summary, rtgs_summary, cash_mode_validation_df, delay_summary, rcpt_cxn_df, report_date,
    )
    date_str = report_date or datetime.now().strftime("%d-%b-%Y")

    rms_full = receipt_made_summary["full"]
    rms_items = "".join(
        f"<li>{group['status'].title()}: <b>{sum(group['totals'].values()):,}</b> receipt(s)</li>"
        for group in rms_full["groups"]
    )

    rtgs_grand = rtgs_summary["matrix"]["grand_total"]
    rtgs_items = "".join(
        f"<li>{TAT_DISPLAY[b]}: <b>{rtgs_grand['by_bucket'][b]['count']:,}</b></li>" for b in TAT_ORDER
    )

    cmv_count = len(cash_mode_validation_df)
    delay_grand = delay_summary["matrix"]["grand_total"]
    delay_items = "".join(
        f"<li>{TAT_DISPLAY[b]}: <b>{delay_grand['by_bucket'][b]['count']:,}</b></li>" for b in TAT_ORDER
    )
    cxn_count = len(rcpt_cxn_df)

    body_font = "font-family:Segoe UI,Arial,sans-serif;font-size:14px;color:#1f2937;line-height:1.6;"
    section = "margin:24px 0;"
    heading = "margin:0 0 8px 0;color:#0f172a;"
    img_style = "max-width:720px;width:100%;border:1px solid #e2e8f0;border-radius:8px;margin-top:12px;display:block;"

    html_body = f"""
    <div style="{body_font}">
      <p>Hi,</p>
      <p>Quick summary of today's DCR processing ({date_str}). Full workbook attached separately.</p>

      <div style="{section}">
        <h3 style="{heading}">1. Receipt Made Summary</h3>
        <ul style="margin:0;padding-left:20px;">
          {rms_items}
          <li>Grand Total: <b>{rms_full['grand_total']:,}</b> receipt(s)</li>
        </ul>
        <img src="cid:{receipt_made_cid}" style="{img_style}" alt="Receipt Made Report" />
      </div>

      <div style="{section}">
        <h3 style="{heading}">2. RTGS Summary</h3>
        <ul style="margin:0;padding-left:20px;">
          <li>Total Updated RTGS Receipts: <b>{rtgs_grand['total_count']:,}</b></li>
          {rtgs_items}
          <li>Total Value: <b>Rs {rtgs_grand['total_value']:.2f} Cr</b></li>
        </ul>
        <img src="cid:{rtgs_cid}" style="{img_style}" alt="RTGS Summary (incl. Online Payment channels)" />
      </div>

      <div style="{section}">
        <h3 style="{heading}">3. Cash Mode Validation Summary</h3>
        <p style="margin:0;">{f"{cmv_count:,} violation(s) found" if cmv_count else "No violations found. Fully compliant."}</p>
      </div>

      <div style="{section}">
        <h3 style="{heading}">4. Delay in Recepting Summary</h3>
        <ul style="margin:0;padding-left:20px;">
          <li>Total Receipts: <b>{delay_grand['total_count']:,}</b></li>
          {delay_items}
        </ul>
      </div>

      <div style="{section}">
        <h3 style="{heading}">5. Receipt Cancellation Summary</h3>
        <p style="margin:0;">{f"{cxn_count:,} receipt(s) cancelled this period" if cxn_count else "No cancelled receipts this period."}</p>
      </div>

      <p>Regards,<br/>DHC Working Automation</p>
    </div>
    """
