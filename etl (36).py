...
def build_mail_html_summary(
    ...
    HTML — with each section's own dashboard image sitting right under
    its bullet points (Receipt Made Report under section 1, RTGS Summary
    — which already includes its own Online Payment Channels panel, there
    isn't a separate online-payment export — under section 2, Delay in
    Recepting under section 4, and the Cancelled-Receipts-by-Mode chart
    under section 5), so each picture sits next to the text it
    illustrates instead of piled at the end. Section 3 (Cash Mode
    Validation) stays text-only, as before.

    img_width is both the CSS width and the HTML width attribute on each
    <img> — Outlook's classic HTML renderer (it's Word's engine) honours
    the width attribute more reliably than max-width alone, so both are
    set to actually keep the images small in the mail rather than just in
    a browser preview.
    ...
    def img_tag(cid: str, alt: str) -> str:
        return f'<img src="cid:{cid}" width="{img_width}" style="{img_style}" alt="{alt}" />'
