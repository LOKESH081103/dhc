...
def generate_airtel_dashboard(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_filtered = df_dash[df_dash['Dash_MOP'].isin(['Deposited - Airtel', 'Deposited - Bank'])].copy()
    ...
    region_pivot, region_cols = _build_pivot(df_filtered, ['Zone_Clean', 'Region_Clean'], ["Zone", "Region"])
    ...
    # --- Region-wise table: zone header row, then its regions in the
    # specified order, then any leftover regions for that zone, then the
    # next zone — not a flat alphabetical sort. ---
    region_flat = region_pivot.reset_index().rename(columns={'Zone_Clean': 'Zone', 'Region_Clean': 'Region'})
    blocks = []
    zones_present = list(dict.fromkeys(region_flat['Zone']))  # preserves first-seen order as a fallback
    ordered_zones = [z for z in ZONE_ORDER if z in zones_present] + [z for z in zones_present if z not in ZONE_ORDER]
    for zone in ordered_zones:
        zdf = region_flat[region_flat['Zone'] == zone]
        # Zone header row = that zone's own subtotal from the zone-wise table
        header = zone_pivot[zone_pivot['Zone'] == zone].copy()
        if not header.empty:
            header['Region'] = ''
            blocks.append(header)
        known = ZONE_REGION_ORDER.get(zone, [])
        placed = set()
        for region in known:
            match = zdf[zdf['Region'].str.upper() == region.upper()]
            if not match.empty:
                m = match.copy()
                m['Region'] = region  # canonical display spelling/case
                blocks.append(m)
                placed.update(match.index)
        leftover = zdf[~zdf.index.isin(placed)]
        if not leftover.empty:
            blocks.append(leftover)
    region_df = pd.concat(blocks, ignore_index=True) if blocks else region_flat.iloc[0:0]
    # Reuse the already-correct zone-level grand total rather than summing
    # region_df directly — region_df intentionally repeats each zone's
    # subtotal as its own header row above that zone's regions, so a
    # straight sum of region_df would double-count every zone.
    region_gt = zone_gt.copy()
    region_gt['Zone'] = 'PAN INDIA'
    region_gt['Region'] = ''
    region_df = pd.concat([region_df, region_gt], ignore_index=True)
...
def _region_table_html(region_df: pd.DataFrame) -> str:
    """
    Table 2 of the reference dashboard: each zone's own subtotal as a bold
    white header row, its regions listed (indented, peach background)
    directly underneath, then the next zone — exactly the grouping shown
    in the reference screenshot — ending in a dark-green PAN INDIA total.
    """
    header_row1 = (
        _dash_th("ZONE/REGION", rowspan=2)
        + _dash_th("Deposited - Airtel", colspan=4)
        + _dash_th("Deposited - Bank", colspan=4)
    ...
    for _, row in region_df.iterrows():
        ...
        is_total = zone.strip().upper() == "PAN INDIA"
        is_zone_header = (not is_total) and (region.strip() == "")

        if is_total:
            bg, color, label, align = _DASH_GREEN, "#ffffff", zone, "center"
        elif is_zone_header:
            bg, color, label, align = "#ffffff", "#111827", zone, "center"
        else:
