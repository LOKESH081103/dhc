...
def _write_brs_status_sheet(ws, result: dict):
    ...
    kpi_row = 4
    n_kpi = len(kpi_fills)
    span = max(last_col // n_kpi, 1)
    col = 1
    for label, fill in kpi_fills.items():
        end_col = min(col + span - 1, last_col)
    ...
    for j in range(n_buckets):
        ws.column_dimensions[get_column_letter(first_bucket_col + j)].width = 12
...
def brs_status_image_bytes(result: dict) -> io.BytesIO:
    ...
    for label, cnt in tally_rows:
        ...
        icon(lx0 + 30, y + sub_row_h / 2, 13, green)
    ...
    dash = "\u2022"
    heading = f"{dash} OVERALL BRS DISTRIBUTION {dash}"
    ...
    for ops in OPS_BRS_ORDER:
        ...
        start += sweep
        legend_items.append((ops, color, ops_totals[ops]))

    draw.ellipse([cx - r_inner, cy - r_inner, cx + r_inner, cy + r_inner], fill="white")
    icon(cx, cy - 22, 20, navy)
    draw.text((cx, cy + 14), f"{grand_total:,}", font=_img_font(26), fill=navy, anchor="mm")
    draw.text((cx, cy + 40), "TOTAL RECEIPTS", font=_img_font(10), fill=muted, anchor="mm")

    ly = cy + r_outer + 50
    lx = rx0 + 20
    for ops, color, cnt in legend_items:
        draw.ellipse([lx, ly - 6, lx + 12, ly + 6], fill=color)
        draw.text((lx + 20, ly), ops, font=_img_font(12), fill=ink, anchor="lm")
        sub = f"({pct(cnt)})"
