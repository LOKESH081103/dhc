...
def filter_by_broad_zone(df: pd.DataFrame, zone_col: str, zone: str) -> pd.DataFrame:
    """zone='Overall' (or falsy) returns df unchanged; otherwise filters to that broad zone."""
...
def build_mail_html_summary(
    ...
    auto_manual_box = _auto_manual_box("Pay Mode RTGS — Auto Receipt / Manual", rtgs_auto_manual)
    ...
    <div style="{body_font}">
      ...
      <div style="{section}margin-top:0;">
        {img_tag(overview_cid, "Receipts Overview")}
      </div>

      <div style="{section}">
        {img_tag(sync_cid, "Receipt Sync Status")}
      </div>
        ...
        <table cellpadding="0" cellspacing="0" style="margin-top:12px;">
          <tr>
            <td style="padding-right:{pair_gap}px;vertical-align:top;">
              ...
              <div style="font-size:11px;color:#6b7280;margin-top:4px;">Receipt Particulars</div>
            </td>
            <td style="vertical-align:top;">
              {img_tag(zone_overview_cid, "Zone Overview — Mode split, TAT exceeded, Bounced/Cancelled", pair_w)}
              <div style="font-size:11px;color:#6b7280;margin-top:4px;">Zone Overview — Mode split, TAT exceeded, Bounced/Cancelled</div>
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="{section}">
        ...
        <ul style="margin:0;padding-left:20px;">
          <li>Cash Mode – Delay in Deposition Count is <b>{cash_brs['delay']['delay_deposition'] if cash_brs else (air_brs['delay']['delay_deposition'] if air_brs else fill)}</b>.</li>
          <li>Cash Mode – Delay in Receipting Count is <b>{cash_brs['delay']['delay_receipting'] if cash_brs else (air_brs['delay']['delay_receipting'] if air_brs else fill)}</b>.</li>
...
def brs_status_image_bytes(result: dict) -> io.BytesIO:
    ...
    for label, cnt in tally_rows:
        ...
        draw.text((lx0 + left_w - 58 + pw / 2, y + sub_row_h / 2), pct(cnt), font=_img_font(11), fill=navy, anchor="mm")
        y += sub_row_h + 10
    ...
    for ops in OPS_BRS_ORDER:
        ...
        draw.text((lx0 + left_w - 76 + pw / 2, y + main_row_h / 2), pct(ops_totals[ops]), font=_img_font(12), fill="white", anchor="mm")
        y += main_row_h + 10
...
def brs_status_image_bytes_v2(result: dict) -> io.BytesIO:
    ...
    draw.text((width / 2, header_h / 2), title.upper(),
              font=_img_font(18), fill="white", anchor="mm")
    ...
    for label, cnt in tally_rows:
        ...
        # Person icon
        cx_icon, cy_icon = pad + 36, y + card_h // 2
        r = 13
        ...
        # Label
        draw.text((pad + 64, y + card_h // 2), label,
                  font=_img_font(14), fill=ink, anchor="lm")

        # Count
        draw.text((pad + left_w - 148, y + card_h // 2),
                  f"{cnt:,}", font=_img_font(17), fill=navy, anchor="lm")

        # % pill
        pw = 68
        pill_x = pad + left_w - 76
        draw.rounded_rectangle([pill_x, y + card_h // 2 - 15,
                                 pill_x + pw, y + card_h // 2 + 15],
                                radius=14, fill=color)
        draw.text((pill_x + pw // 2, y + card_h // 2), pct_val,
                  font=_img_font(12), fill="white", anchor="mm")
    ...
    draw.text((rx0 + right_w / 2, ry0 + 22),
              ...
              font=_img_font(15), fill=navy, anchor="mm")
    ...
    for label, cnt in tally_rows:
        ...
        if cnt > 0 and frac > 0.015:
            ...
            label_text = f"{cnt:,}"
            tw = draw.textlength(label_text, font=_img_font(13))
            draw.rounded_rectangle([lx_ - tw / 2 - 8, ly_ - 14,
                                     lx_ + tw / 2 + 8, ly_ + 14],
                                   radius=8, fill=color)
            draw.text((lx_, ly_), label_text,
                      font=_img_font(13), fill="white", anchor="mm")
        start += sweep
        legend_items.append((label, color, cnt))
    ...
    draw.text((cx, cy + 14), f"{tally_total:,}",
              font=_img_font(26), fill=navy, anchor="mm")
    draw.text((cx, cy + 42), "TALLY RECEIPTS",
              font=_img_font(10), fill=muted, anchor="mm")

    # Legend
    leg_y = cy + r_outer + 40
    leg_x = rx0 + 10
    for label, color, cnt in legend_items:
        draw.ellipse([leg_x, leg_y - 6, leg_x + 12, leg_y + 6], fill=color)
        draw.text((leg_x + 20, leg_y), label,
                  font=_img_font(12), fill=ink, anchor="lm")
        sub = f"{cnt:,}  ({cnt / tally_total * 100:.1f}%)"
        draw.text((leg_x + 20, leg_y + 17), sub,
                  font=_img_font(10), fill=muted, anchor="lm")
        leg_y += 46
