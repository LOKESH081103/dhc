...
def load_dcr(file) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # STATUS IN TAB uses the literal code "NA" to mean Pending. Pandas'
    # default NA-value list treats the bare text "NA" as missing on every
    # read, so every Pending receipt was silently turning blank before
    # STATUS_MAP ever saw it. keep_default_na=False fixes that — but
    ...
    # object dtype and crashing pd.to_datetime). So: re-read ONLY this one
    # column with protection on, and patch it back in by row position.
    if "STATUS IN TAB" in receipts.columns:
        if hasattr(file, "seek"):
            file.seek(0)
        status_col = pd.read_excel(
            file, sheet_name="Sheet1", engine="pyxlsb",
            usecols=["STATUS IN TAB"], keep_default_na=False,
        )["STATUS IN TAB"]
        status_col = status_col.replace("", np.nan)  # genuine blanks still count as missing
...
def receipt_made_summary_to_image_bytes(df: pd.DataFrame) -> io.BytesIO:
    """
    Chart-based dashboard PNG: a donut chart for the overall status split,
    KPI stat panels beside it, and a stacked horizontal bar chart breaking
    each payment mode down by status. Built with matplotlib for crisp,
    properly-weighted typography and real data visualization rather than
    a grid of coloured boxes.
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    n_modes = max(len(mode_rows), 1)

    fig_h = 7.6 + max(0, n_modes - 4) * 0.55
    fig = plt.figure(figsize=(12, fig_h), dpi=160, facecolor=bg)

    fig.text(0.055, 0.965, "RECEIPT MADE SUMMARY", fontsize=23, fontweight="bold", color=navy)
    fig.text(0.055, 0.932, "By Payment Mode & Receipt Status", fontsize=11.5, color="#5A6472")
    fig.text(
        0.055, 0.912,
        f"Source: STATUS IN TAB (C/NA/X)  ·  {datetime.now().strftime('%d-%b-%Y %H:%M')}",
        fontsize=9, color="#9099A6",
    )

    top_h = 1.05
    bar_h = 1.55 if n_modes <= 4 else 1.75
    gs = fig.add_gridspec(
        2, 3, height_ratios=[top_h, bar_h],
        top=0.865, bottom=0.10, left=0.055, right=0.97, hspace=0.55, wspace=0.28,
    )

    # ── Donut: overall status split ──
    ax_donut = fig.add_subplot(gs[0, 0])
    values = [int(grand[c]) for c in status_cols]
    colors = [status_colors.get(c, "#7F8C8D") for c in status_cols]
    ax_donut.pie(
        values, colors=colors, startangle=90,
        wedgeprops=dict(width=0.40, edgecolor=bg, linewidth=3),
    )
    ax_donut.text(0, 0.10, f"{grand_total:,}", ha="center", va="center", fontsize=19, fontweight="bold", color=navy)
    ax_donut.text(0, -0.16, "TOTAL RECEIPTS", ha="center", va="center", fontsize=7.5, color="#8b95a1")
    ax_donut.set_aspect("equal")

    # ── KPI stat cards: one per status ──
    ax_kpi = fig.add_subplot(gs[0, 1:])
    ax_kpi.axis("off")
    ax_kpi.set_xlim(0, 1)
    ax_kpi.set_ylim(0, 1)
    n = max(len(status_cols), 1)
    box_w = 1.0 / n
    for i, sc in enumerate(status_cols):
        val = int(grand[sc])
        pct = val / grand_total * 100
        x0 = i * box_w + 0.015
        color = status_colors.get(sc, "#7F8C8D")
        card = FancyBboxPatch(
            (x0, 0.08), box_w - 0.03, 0.84,
            boxstyle="round,pad=0.006,rounding_size=0.035",
            linewidth=0, facecolor="white",
            path_effects=[pe.withSimplePatchShadow(offset=(1.2, -2.2), shadow_rgbFace="#0F203C", alpha=0.10)],
        )
        ax_kpi.add_patch(card)
        ax_kpi.add_patch(mpatches.Rectangle((x0 + 0.028, 0.66), 0.045, 0.14, facecolor=color, linewidth=0))
        ax_kpi.text(x0 + 0.028, 0.46, f"{val:,}", fontsize=15.5, fontweight="bold", color="#1c1c1c")
        ax_kpi.text(x0 + 0.028, 0.24, str(sc).upper(), fontsize=8, color="#6b7280")
        ax_kpi.text(x0 + box_w - 0.055, 0.24, f"{pct:.1f}%", fontsize=8.5, color=color, ha="right", fontweight="bold")

    # ── Stacked horizontal bar: mode x status ──
    ax_bar = fig.add_subplot(gs[1, :])
    y_pos = list(range(n_modes))
    left = [0] * n_modes
    for sc in status_cols:
        vals = mode_rows[sc].astype(int).tolist()
        ax_bar.barh(
            y_pos, vals, left=left, color=status_colors.get(sc, "#7F8C8D"),
            label=str(sc), height=0.52, edgecolor=bg, linewidth=2, zorder=3,
        )
        left = [l + v for l, v in zip(left, vals)]

    ax_bar.set_yticks(y_pos)
    ax_bar.set_yticklabels(mode_rows["Mode"].tolist(), fontsize=11, color="#20242c")
    ax_bar.invert_yaxis()
    for spine in ("top", "right", "left"):
        ax_bar.spines[spine].set_visible(False)
    ax_bar.spines["bottom"].set_color("#D8DCE2")
    ax_bar.tick_params(axis="x", colors="#8b95a1", labelsize=9)
    ax_bar.tick_params(axis="y", length=0)
    ax_bar.set_facecolor(bg)
    ax_bar.grid(axis="x", color="#E3E6EA", linewidth=0.8, zorder=0)
    ax_bar.set_axisbelow(True)

    max_total = max(int(mode_rows["Total"].max()), 1) if n_modes else 1
    for i, tot in enumerate(mode_rows["Total"].astype(int).tolist()):
        ax_bar.text(tot + max_total * 0.012, i, f"{tot:,}", va="center", fontsize=10, fontweight="bold", color=navy)
    ax_bar.set_xlim(0, max_total * 1.12)

    ax_bar.set_title("BREAKDOWN BY PAYMENT MODE", loc="left", fontsize=11.5, fontweight="bold", color="#333333", pad=14)
    ax_bar.legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.16 if n_modes <= 4 else -0.11),
        ncol=min(len(status_cols), 6), frameon=False, fontsize=9.5,
    )

    fig.text(0.055, 0.02, "Generated automatically — DHC Working Automation", fontsize=8, color="#9099A6")
