...
def rtgs_summary_to_image_bytes(summary: dict) -> io.BytesIO:
    ...
    for block in shown_zones:
        ...
        draw.text((pad, y + 14), str(zname), font=_img_font(13), fill=(30, 30, 30))

        bar_w = int(bar_max_w * (total / max_zone_count))
        by0, by1 = y + 4, y + row_h - 4
        ...
        if total:
            for b in TAT_ORDER:
                ...
                if seg_w > 0:
                    draw.rectangle([seg_x, by0, seg_x + seg_w, by1], fill=_TAT_COLORS[b])
                    seg_x += seg_w
        draw.text((bar_x0 + bar_max_w + 12, y + 14), f"{total:,}", font=_img_font(13), fill=(30, 30, 30))
        y += row_h + row_gap
...
def delay_summary_to_image_bytes(summary: dict) -> io.BytesIO:
    ...
    height = (
        header_h + gap + kpi_h + gap + header_row_h
        + len(shown) * row_h + (26 if extra else 0) + gap + 60 + 40
    )
    ...
    for b in TAT_ORDER:
        draw.text((cx + 10, y + 10), TAT_DISPLAY[b], font=_img_font(12), fill=(60, 60, 60))
        cx += col_w
    draw.text((cx + 10, y + 10), "ZONE TOTAL", font=_img_font(12), fill=(60, 60, 60))
    y += header_row_h
    ...
    for block in shown:
        ...
        draw.text((grid_x0, y + 12), str(zname), font=_img_font(12), fill=(30, 30, 30))
        cx = grid_x0 + row_label_w
        for b in TAT_ORDER:
            cnt = st["by_bucket"][b]["count"]
            share = cnt / total
            color = heat_color(share)
            draw.rectangle([cx, y, cx + col_w - 6, y + row_h - 6], fill=color)
            txt_color = "white" if share > 0.55 else (30, 30, 30)
            draw.text((cx + 10, y + 10), f"{cnt:,} ({share * 100:.0f}%)", font=_img_font(11), fill=txt_color)
            cx += col_w
        draw.text((cx + 10, y + 10), f"{st['total_count']:,}", font=_img_font(12), fill=(30, 30, 30))
        y += row_h
