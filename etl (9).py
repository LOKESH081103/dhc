        ...
        if total:
            for b in TAT_ORDER:
                ...
                if seg_w > 0:
                    ...
                    if seg_w >= 24 and cnt > 0:
                        draw.text(
                            (seg_x + seg_w / 2, (by0 + by1) / 2), f"{cnt:,}",
                            font=_img_font(11), fill="white", anchor="mm",
                        )
                    seg_x += seg_w
        draw.text((bar_x0 + bar_max_w + 12, (by0 + by1) / 2), f"{total:,}", font=_img_font(14), fill=(20, 20, 20), anchor="lm")
        draw.text((pad, y + 30), "  ".join(f"{TAT_DISPLAY[b]}: {st['by_bucket'][b]['count']:,}" for b in TAT_ORDER), font=_img_font(9), fill=(130, 130, 130))
        y += row_h + row_gap
